...
def create_refactor_plan(repo_root: str, goal: str, context_files: List[str]) -> str:
    # include selected files (clipped)
    parts = []
...
"""
    ...
    # safety: if model returns empty or super short, keep old
    if not new.strip():
        new = old
    diff = unified_diff(old, new, filename=rel_path)
    return {"old": old, "new": new, "diff": diff}
...
def ensure_state():
    ...
    st.session_state.setdefault("tasks", []) # list of tickets
    st.session_state.setdefault("last_plan", "")
    st.session_state.setdefault("pending_patch", None)  # dict with old/new/diff/path
...
def main():
    ...
    # Top title matches the idea in the screenshot
    ...
    # Layout: left = file view, right = chat + actions
    ...
    with left:
        ...
        if st.button("Generate architecture diagram text (Mermaid)"):
            # give model file list (not entire repo) to avoid huge prompts
...
"""
    ...
    with right:
        ...
        if user_msg:
            ...
            # lightweight context: show repo tree sample + selected file snippet
            file_snip = ""
            if selected_file and selected_file != "(no files)":
...
"""
        ...
        with c1:
            if st.button("Create plan"):
                ...
                # naive ticket extraction (optional): keep as plain text tasks
                # You can manually add tasks from plan as well.
                st.success("Plan created ✅")
                st.rerun()

        with c2:
            patch_instructions = st.text_area(
                "Patch instructions for selected file",
                value="",
                height=100
            )
            if st.button("Generate patch (with diff)"):
                if not selected_file or selected_file == "(no files)":
                    ...
                    patch = generate_patch(repo_root, selected_file, patch_instructions or plan_goal or "Refactor for clarity")
                    patch["path"] = selected_file
        ...
        if pending:
            ...
            if st.button("Apply patch"):
                ...
                st.success("Patch applied ✅ (file updated in extracted repo)")
                st.rerun()
